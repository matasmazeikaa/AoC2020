const passportData = `iyr:1928 cid:150 pid:476113241 eyr:2039 hcl:a5ac0f
ecl:#25f8d2
byr:2027 hgt:190
...
const allowedKeys = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid'];
const passports = passportData.split('\n\n').map((passport) => passport);

const isAllKeysValid = (keys, string) => {
	const truthArray = Array(keys.length);

	allowedKeys.forEach((key, index) => string.includes(key) ? truthArray[index] = true : truthArray[index] = false);

	return truthArray.every((boolean) => boolean === true);
}

const isBYRValid = (byr) => (byr >= 1920 && byr <= 2002);
const isIYRValid = (iyr) => iyr >= 2010 && iyr <= 2020;
const isEYRValid = (eyr) => eyr >= 2020 && eyr <= 2030;
const isHGTValid = (hgt) => {
	if (!(hgt.includes('cm') || hgt.includes('in'))) {
		return false
	}

	const plainHeight = Number(hgt.replace(/in|cm/g, ''));

	if (hgt.includes('cm') && plainHeight >= 150 && plainHeight <= 193) {
		return true
	}

	if (hgt.includes('in') && plainHeight >= 59 && plainHeight <= 76) {
		return true
	}

	return false
}
const isHCLValid = (hcl) => /#([a-f\d]){6}$/i.test(hcl);
const isECLValid = (ecl) => ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].some((eyeColor) => ecl.includes(eyeColor));
const isPIDValid = (pid) => /^\d{9}$/.test(pid);

const extractHCL = (passport) => passport.match(/hcl[^\s]+/g)[0].split(':')[1];
const extractBYR = (passport) => passport.match(/byr[^\s]+/g)[0].split(':')[1];
const extractIYR = (passport) => passport.match(/iyr[^\s]+/g)[0].split(':')[1];
const extractEYR = (passport) => passport.match(/eyr[^\s]+/g)[0].split(':')[1];
const extractHGT = (passport) => passport.match(/hgt[^\s]+/g)[0].split(':')[1];
const extractECL = (passport) => passport.match(/ecl[^\s]+/g)[0].split(':')[1];
const extractPID = (passport) => passport.match(/pid[^\s]+/g)[0].split(':')[1];

const solutionOne = () => {
	let validPassportCount = 0;

	passports.forEach((passport) => {
		if (isAllKeysValid(allowedKeys, passport)) {
...
const solutionTwo = () => {
	...
	passports.forEach((passport) => {
		if (isAllKeysValid(allowedKeys, passport)) {
			if (
				isBYRValid(Number(extractBYR(passport))) &&
				isIYRValid(Number(extractIYR(passport))) &&
				isEYRValid(Number(extractEYR(passport))) &&
				isHGTValid(extractHGT(passport)) &&
				isHCLValid(extractHCL(passport)) &&
				isECLValid(extractECL(passport)) &&
				isPIDValid(extractPID(passport))
			) {
				validPassportCount += 1
			}
		}
